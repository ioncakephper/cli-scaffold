...
test('uses default.config.js when no --config is provided', () => {
  const res = spawnSync(process.execPath, [cliPath, 'hello'], { encoding: 'utf8' });
...
test('loads config when --config points to a file relative to CWD', () => {
  ...
  fs.writeFileSync(tmpConfigPath, "module.exports = { hello: { name: 'RelName', title: 'Prof.' } };\n");

  try {
    // pass relative path from CWD
    const res = spawnSync(process.execPath, [cliPath, '--config', path.join('test', tmpConfigName), 'hello'], { encoding: 'utf8' });
    const out = (res.stdout || '') + (res.stderr || '');
